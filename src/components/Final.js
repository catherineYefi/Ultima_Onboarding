 ...
 * Ожидаемые (необязательные) поля в content.sections.final:
 * {
 *   title?: string,
 *   lead?: string,
...
export default function Final({ content, scrollToSection }) {
  ...
  const title = fin?.title ?? "Выпускной: финальные артефакты и защита результата";
  const lead =
    fin?.lead ??
  ...
  const deliverables = Array.isArray(fin?.deliverables) && fin.deliverables.length > 0
    ? fin.deliverables
    : [
  ...
  const checklist = Array.isArray(fin?.checklist) && fin.checklist.length > 0
    ? fin.checklist
    : [
  ...
  const criteria = Array.isArray(fin?.criteria) && fin.criteria.length > 0
    ? fin.criteria
    : [
